...
def get_named_beta_schedule(args):
    ...
    if schedule_name == "linear":
        ...
        return beta_noise_schedule(
            num_diffusion_timesteps, a=args.beta_a, b=args.beta_b
        )
        # pass
    else:
...
def exponential_mapping(x, v):
    """
    Geodesic exponential mapping for hyperbolic space.
    exp_x[v] = cos(||v||) * x + sin(||v||) * (v / ||v||)
    """
    norm_v = v.norm(p=2, dim=-1, keepdim=True)
    v_unit = v / (norm_v + 1e-8)
    cos_v = torch.cos(norm_v)
    sin_v = torch.sin(norm_v)
    return cos_v * x + sin_v * v_unit
